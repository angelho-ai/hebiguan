...
姓名（必填）、聯絡電話（必填）、LINE ID（選填）、看屋日期、時段、個資同意（必須勾選）。選擇「其他時間」才顯示選填的「希望看屋時間」。不再收集買方電子信箱。
送出後寫入 Google Sheet「預約」工作表，並嘗試寄通知信至指定信箱。畫面顯示收件確認與所選場次，仍須後續人工聯繫確認看屋。
試算表既有的「稱謂」與「Email」欄位及歷史資料保留，但新預約這兩欄留空，本名儲存在「姓名」欄。姓名欄為使用者自行填寫，並非身分驗證。寄給屋主的預約通知信不受影響。
